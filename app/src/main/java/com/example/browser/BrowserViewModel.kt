...
class BrowserViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val quickShortcuts: StateFlow<List<QuickShortcutItem>> = combine(customShortcuts, topVisitedSites) { custom, top ->
        ...
        top.forEach { topSite ->
            ...
            if (list.none { UrlUtils.extractDomain(it.url) == domain || it.url == topSite.url }) {
                ...
                if (!_isPrivateMode.value) {
                    if (tabs.isEmpty()) {
                        ...
                        val initialTab = BrowserTab(
                            id = _activeTabId.value.ifBlank { UUID.randomUUID().toString() },
                            profileId = profileId,
                            url = "",
                            title = "New Tab",
                            isPrivate = false
